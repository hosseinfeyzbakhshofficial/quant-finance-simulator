...
with st.sidebar.expander("⚙️ Market Parameters", expanded=True):
    S0 = st.number_input(
        ...
        value=100.0,
        step=1.0,
        help="Current spot price of the underlying asset.",
    )
    STRIKE = st.number_input(
        "Option Strike Price (K)",
        value=100.0,
        step=1.0,
        help="Predetermined price at which the option can be exercised.",
    )
    RISK_FREE_RATE = st.slider(
        ...
        0.20,
        0.05,
        help="Annualized risk-free interest rate (e.g., US Treasury yield).",
    )

with st.sidebar.expander("📊 Stochastic Process (GBM)", expanded=True):
    MU = st.slider(
        "Drift (μ)", 0.00, 0.30, 0.10, help="Expected annualized return of the asset."
    )
    SIGMA = st.slider(
        "Volatility (σ)",
        0.01,
        1.00,
        0.20,
        help="Annualized standard deviation of asset returns.",
    )
    T = st.slider(
...
with st.sidebar.expander("🧮 Monte Carlo Settings", expanded=False):
    N_SIMULATIONS = st.number_input(
        ...
        value=5000,
        step=1000,
        help="Higher paths increase accuracy but require more compute power.",
    )
    DT = st.number_input(
        "Time Step (dt)",
        value=0.01,
        format="%.3f",
        help="Granularity of the simulation steps.",
    )
    seed_toggle = st.checkbox(
        "Use Fixed Seed",
        value=True,
        help="Locks the random number generator for reproducible results.",
    )
    if seed_toggle:
        np.random.seed(42)
...
with st.spinner("Initializing Quantitative Engine & Simulating Paths..."):
    ...
    mc_price = monte_carlo_option_price(
        paths=mc_paths, strike=STRIKE, r=RISK_FREE_RATE, T=T
    )
...
with tab1:
    ...
    col2.metric(
        "Black-Scholes Val", f"${bs_price:.4f}", "Theoretical Model", delta_color="off"
    )
    col3.metric(
        "Moneyness (S/K)",
        f"{(S0 / STRIKE):.4f}",
        "ITM" if S0 > STRIKE else "OTM",
        delta_color="normal",
    )
...
with tab4:
    ...
    for i in range(X.shape[0]):
        for j in range(X.shape[1]):
            implied_vol = SIGMA + 0.0005 * (X[i, j] - STRIKE) ** 2
...
with tab5:
    ...
    for n_paths in simulation_sizes:
        ...
        final_payoffs = np.maximum(temp_paths[:, -1] - STRIKE, 0)

        discounted_payoffs = np.exp(-RISK_FREE_RATE * T) * final_payoffs
        ...
        pricing_errors.append(abs(mc_temp_price - bs_price))
    ...
    col_a.metric("Final Pricing Error", f"{pricing_errors[-1]:.6f}")
    ...
    col_c.metric(
        "Simulation Stability", "HIGH" if pricing_errors[-1] < 0.5 else "MODERATE"
    )
...
with tab6:
    ...
    def smile_model(k, a, b, c):
        return a * (k - STRIKE) ** 2 + b * (k - STRIKE) + c
    ...
    fitted_vols = smile_model(fitted_strikes, a_fit, b_fit, c_fit)
    ...
    c1.metric("Quadratic Term (a)", f"{a_fit:.6f}")

    c2.metric("Linear Term (b)", f"{b_fit:.6f}")

    c3.metric("Base Volatility (c)", f"{c_fit:.4f}")
...
with tab7:
    ...
    rolling_return = returns.rolling(window=10).mean() * 252
    ...
    r1.metric("Sharpe Ratio", f"{sharpe_ratio:.3f}")

    r2.metric("Max Drawdown", f"{max_drawdown:.2%}")

    r3.metric("VaR 95%", f"{var_95:.2%}")

    r4.metric("VaR 99%", f"{var_99:.2%}")
...
with tab8:
    ...
    df_export = pd.DataFrame(
        mc_paths[:display_limit].T, columns=[f"Path_{i}" for i in range(display_limit)]
    )
...
if isinstance(loaded_data, dict):
    ...
    c1.metric("Cached MC Price", f"${loaded_data.get('mc_price', 0):.4f}")

    c2.metric("Cached BS Price", f"${loaded_data.get('bs_price', 0):.4f}")

    c3.metric("Cached Error", f"{loaded_data.get('pricing_error', 0):.6f}")
...
if run_live:
    ...
    while True:
        ...
        tick_metric.metric(
            "Current Spot", f"${st.session_state.current_price:.2f}", f"{shock:+.2f}"
        )
        time.sleep(1)
...
st.sidebar.metric("CPU Load", "92%")

st.sidebar.metric("Monte Carlo Throughput", f"{N_SIMULATIONS:,}/sec")

st.sidebar.metric("Latency", "12 ms")
st.sidebar.divider()
...
if st.sidebar.button("Load Previous Session"):
    ...
    if isinstance(loaded_data, dict):
        ...
        st.sidebar.write(f"Loaded MC Price: {mc_price:.4f}")

    else:
        st.sidebar.warning("No cached session found.")
